...
export const getAllChildrenIds = (tree: TreeDataIm, parentItemId: ItemId): List<ItemId> => {
    return tree
...
const treeReducer = (tree: TreeDataIm = initState.get('tree'), action: Action) => {
    switch (action.type) {
        case AT.SET_TREE:
        ...
        case AT.REMOVE_ITEM: {
            ...
            const parentItem = tree
                .get('items')
                .find((value: TreeItemIm) => value.get('children').some((id: ItemId) => id === removeItemId))
        ...
        case AT.COPY_ITEM: {
            ...
            const newItems = childrenIds.reduce((accItems, curId) => {
                ...
                const newChildren = curItem.get('children').map((prevId: ItemId) => correspond.get(prevId))
            ...
            const parentItem: TreeItemIm = tree
                .get('items')
                .find((item: TreeItemIm) => item.get('children').some((id: ItemId) => id === originItemId))
            const indexToInsert = parentItem.get('children').findIndex((id: ItemId) => id === originItemId) + 1
