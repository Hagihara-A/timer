...
export const getNewItemIds = (tree: TreeData | TreeDataIm, numIds: number) => {
  const startNum = Map.isMap(tree)
    ? tree.get("items").size - 1
...
export const getParentItem = (tree: TreeData, childId: ItemId) => {
  const parentItemEnt = Object.entries(tree.items).filter(ent =>
    ent[1].children.includes(childId)
  );
...
export const removeItemFromTree = (tree: TreeData, itemId: ItemId) => {
  ...
  return mutateTree(tree, parentItem.id, {
    children: newChildren,
    hasChildren: newChildren.length > 0
  });
};
export const combineTwoTimersToSection = (
  tree: TreeData,
  ...
  const newChildren = [...dstParentItem.children].map(id =>
    id === dstItemId ? newSectionId : id
  );
